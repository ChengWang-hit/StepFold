...
def main():
    ...
    # --- Initialize WandB ---
    if local_rank == 0:
        run_name = f"StepFold_S2_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    train_data1 = DataGenerator(config['train_data_dir1'], 'train_max600_with_indices', mode='train')
    ...
    global_mean_len = train_data1.mean_len
    ...
    test_data = DataGenerator(config['test_data_dir'], 'TS123_hard_with_indices', mode='test')
...
if __name__ == '__main__':
    main()
    # Running in the terminal: CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --standalone --nproc_per_node=4 code/train_S2.py