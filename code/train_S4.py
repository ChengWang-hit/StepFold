...
def main():
    ...
    parser.add_argument('--training_config_path', type=str, default='configs/S4.json', help='Path to the training config file')
    ...
    # --- Initialize WandB ---
    if local_rank == 0:
        run_name = f"StepFold_S4_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    test_data = DataGenerator(config['test_data_dir'], 'test_with_indices', mode='test')
...
if __name__ == '__main__':
    main()
    # Running in the terminal: CUDA_VISIBLE_DEVICES=5,6 torchrun --standalone --nproc_per_node=2 code/train_S4.py